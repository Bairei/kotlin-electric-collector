...
@Repository
class CollectorQueryReactiveRepositoryImpl(val mongoTemplate: ReactiveMongoTemplate) : CollectorQueryReactiveRepository {
    ...
    override fun findByReadingBetween(from: Int, to: Int): Flux<CollectorEntity> {
        ...
        val result = mongoTemplate.query(CollectorEntity::class.java)
                .matching(query(where(CollectorEntity::collectorReading).gte(from).lte(to)))
                .all()
